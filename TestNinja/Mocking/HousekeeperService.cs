...
namespace TestNinja.Mocking
{
    public class HousekeeperService
    {
        ...
        private readonly IStatementRegistration _statementRegistration;
        private readonly IEmailFileSender _emailFileSender;
        private readonly IXtraMessageBox _messageBox;

        public HousekeeperService(
            IUnitOfWork unitWork, 
            IStatementRegistration statementRegistration, 
            IEmailFileSender emailFileSender,
            IXtraMessageBox messageBox)
        {
            _unitWork = unitWork;
            _statementRegistration = statementRegistration;
            _emailFileSender = emailFileSender;
            _messageBox = messageBox;
        }

        public void SendStatementEmails(DateTime statementDate)
        {
            var housekeepers = _unitWork.Query<Housekeeper>();
            ...
            foreach (var housekeeper in housekeepers)
            {
                ...
                var statementFilename = _statementRegistration.SaveStatement(housekeeper.Oid, housekeeper.FullName, statementDate);
                ...
                try
                {
                    _emailFileSender.EmailFile(emailAddress, emailBody, statementFilename,
